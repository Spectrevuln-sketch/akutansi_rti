          ...
          <div class="card-body bg-light" style="border-radius: 10px;">
            <table id="display" class="table table-bordered table-striped table-dark" style="width: 200%; min-height:80%;">
              ...
              <tbody id="EditedInv">
                ...
                $query = $this->db->get_where('invoice', ['agen' => urldecode($this->uri->segment(3))])->result_array();
                ?>
                <?php
                $i = 1;
                foreach ($query as $agen) : ?>
                  <tr>
                    <th scope="row"><?= $i++; ?></th>
                    <td id="tanggal"><?= date('d - M - Y  H:i:s', $agen['created_at']); ?></td>
                    <td id="customer"><?= $agen['customer']; ?></td>
                    <td id="noInv"><?= $agen['no_invoice']; ?></td>
                    <td id="keterangan"><?= $agen['keterangan']; ?></td>
                    <td id="selling"><?= rupiah($agen['selling']); ?></td>
                    <td id="ppn"><?= $agen['ppn']; ?></td>
                    <td id="buying"><?= rupiah($agen['buying']); ?></td>
                    <td id="diskonCustomer"><?= $agen['diskon_customer']; ?></td>
                    <td id="diskonMarketing"><?= $agen['diskon_marketing']; ?></td>
                    <td id="margin"><?= rupiah($agen['margin']); ?></td>
                    <td>
                      <a href="" class="badge badge-warning my-2 modalEditInv" data-toggle="modal" data-target="#edit_inv" data-id="<?= $agen['id']; ?>">Edit</a>
                      <a href="" class="badge badge-danger my-2" data-toggle="modal" data-target="#del_inv<?= $agen['id']; ?>" data-id="<?= $agen['id']; ?>">Delete</a>
                    </td>
                  </tr>
                  <div class="modal fade" id="del_inv<?= $agen['id']; ?>" tabindex="-1" role="dialog" aria-labelledby="largeModal" aria-hidden="true">
                      ...
                      <div class="modal-content">
                        ...
                        <form class="form-horizontal" method="post" action="<?= base_url('user/getDel'); ?>">
                          <div class="modal-body">
                            <p>Are you sure want to delete? <b><?= $agen['customer']; ?></b></p>
                          </div>
                          <div class="modal-footer">
                            <input type="hidden" name="inv_id" value="<?= $agen['id']; ?>">
